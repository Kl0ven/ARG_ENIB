	...
	enigma.findAll({order: ['id']}).then(e => {
		for (var i = 0; i < e.length; i++) {
			dataEnigma.push({
				name: e[i].name,
				id: e[i].id,
				first_time_visited: e[i].first_time_visited != null ? e[i].first_time_visited.toLocaleDateString('fr-FR') + ' ' + e[i].first_time_visited.toLocaleTimeString('fr-FR') : '-',
			...
			winner.findAll({order: ['enigma_id', 'date']}).then(w => {
				for (var i = 0; i < w.length; i++) {
					dataWinner.push({
						...
						name: w[i].name,
						date: w[i].date.toLocaleDateString('fr-FR') + ' ' + w[i].date.toLocaleTimeString('fr-FR')
					});
				}
